    ...
    <html>
      <body style="background: #000; color: #fff; font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0;">
        <div style="text-align: center; padding: 20px; max-width: 300px;">
          ...
          <h2 style="margin: 0 0 10px 0; font-size: 18px; font-weight: 900; text-transform: uppercase; letter-spacing: 1px;">Authorized</h2>
          <p style="margin: 0 0 20px 0; font-size: 12px; color: rgba(255,255,255,0.6); line-height: 1.5;">Syncing with your terminal. This window will close automatically.</p>
          
          <button id="manualSync" style="display: none; width: 100%; padding: 12px; background: #fff; color: #000; border: none; border-radius: 8px; font-weight: 900; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; cursor: pointer;">
            Manual Sync
          </button>
          ...
          <script>
            ...
            // Show manual button after 3 seconds if window is still open
            setTimeout(() => {
...
app.get('/broker/zerodha/orders', async (c) => {
  const username = c.req.query('username');
  if (!username) return c.json({ error: 'Missing username' }, 400);
...
app.get('/broker/status', async (c) => {
  ...
  if (!c.env.DB) {
    console.error('DB binding missing in status check');
    return c.json({ connected: false, error: 'Database binding missing' });
  }
  ...
  try {
    ...
    const user = await userStmt.bind(username.toLowerCase()).first() as { id: number };
    if (!user) {
      console.log(`User ${username} not found for status check`);
